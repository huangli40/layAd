...
class PublicController extends Controller
{
    ...
    public function uploadImg(Request $request)
    {
        ...
        if ( config('UEditorUpload.core.mode') == 'local' ) {
            $newFile= date('Ymd').'/'.$newFile;
            $res    = Storage::disk('public')->put($newFile, file_get_contents($path));
            $url    = strpos(config('app.url'), 'localhost') === false ? config('app.url') :  '';  
            $url    = $url.'/'.config('filesystems.disks.public.root').'/'.$newFile;    
        }else{
            $disk   = QiniuStorage::disk('qiniu');
            $res    = $disk->put($newFile,file_get_contents($path));
